...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/todos", get(todo_controller::list).post(todo_controller::create))
        .route("/todos/completed/delete", post(todo_controller::delete_completed))
    ...
    fn into_response(self) -> Response {
        match self {
            AppError::Service(e) => match &e {
                Error::TodoNotFound(..) => global_exception_handler::handle_not_found(&e),
                _ => {
                    tracing::error!("{e}");

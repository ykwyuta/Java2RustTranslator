...
#[test]
fn integer_arithmetic_wraps_like_java() -> JResult<()> {
...
fn division_by_zero_throws() {
    let e = jrt::num::div_i32(1, 0).unwrap_err();
...
#[test]
fn concat_uses_java_string_conversion() -> JResult<()> {
    let s = jstr!("x=");
...
#[test]
fn string_methods_use_utf16_indices() -> JResult<()> {
    ...
    let e = s.char_at(3).unwrap_err();
    assert!(e.instance_of("java.lang.StringIndexOutOfBoundsException"));
    Ok(())
}

#[test]
fn string_builder_shares_state() -> JResult<()> {
...
#[test]
fn arrays_are_shared_and_bounds_checked() -> JResult<()> {
    let a: JArray<i32> = JArray::new(3)?;
    let b = a.clone();
    b.set(1, 42)?;
    assert_eq!(a.get(1)?, 42);
    let e = a.get(3).unwrap_err();
    assert_eq!(e.to_jstring()?.as_str()?, "java.lang.ArrayIndexOutOfBoundsException: Index 3 out of bounds for length 3");
    assert!(JArray::<i32>::new(-1).err().unwrap().instance_of("java.lang.NegativeArraySizeException"));
    assert!(JArray::<i32>::null().length().unwrap_err().instance_of("java.lang.NullPointerException"));
...
fn math_and_numbers() -> JResult<()> {
    ...
    let e = number::parse_int(&jstr!("12x")).unwrap_err();
    assert_eq!(e.to_jstring()?.as_str()?, "java.lang.NumberFormatException: For input string: \"12x\"");

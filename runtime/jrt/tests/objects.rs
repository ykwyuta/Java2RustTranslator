...
#[test]
fn hash_map_iteration_order_matches_java() {
    let m = new_map(MapKind::Hash);
    let sum = jrt::lambda(&["java.util.function.BiFunction"], |a| box_i32(a[0].unbox_i32() + a[1].unbox_i32()));
...
#[test]
fn priority_queue_layout_matches_java() {
...
#[test]
fn split_matches_java() {
    let show = |a: JArray<JString>| a.to_vec().iter().map(|x| x.to_string()).collect::<Vec<_>>();
...
#[test]
fn exceptions_are_caught_by_try_block() {
    let r = jrt::try_block(|| -> Flow<()> {
        jrt::num::div_i32(1, 0);
        Flow::Normal
    });
    ...
    let r = jrt::try_block(|| -> Flow<i32> { Flow::Return(5) });
    assert!(matches!(r, Ok(Flow::Return(5))));
}

#[test]
fn boxing_cache_matches_java_identity() {
...
#[test]
fn null_string_dereference_throws_npe() {
    let r = jrt::try_block(|| -> Flow<()> {
        JString::null().length();
        Flow::Normal
    });
    assert!(r.err().unwrap().instance_of("java.lang.NullPointerException"));
}
